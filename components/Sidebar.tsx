...
const ITEMS_NAV: ItemNav[] = [
  { icono: FilePenLine, etiqueta: "Nuevo análisis", activo: true },
  { icono: FolderOpen, etiqueta: "Mis análisis" },
  { icono: LayoutTemplate, etiqueta: "Plantillas" },
  { icono: Library, etiqueta: "Biblioteca clínica" },
  { icono: BookOpenCheck, etiqueta: "Guía de uso" },
  { icono: Settings, etiqueta: "Configuración" },
];

/**
 * Los ítems de navegación distintos a "Nuevo análisis" son de próxima
 * disponibilidad: hoy la app solo tiene ese flujo, pero se muestran para
 * comunicar el mapa del producto.
 */
  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col bg-sidebar px-4 py-6 text-white lg:flex print:hidden">
      ...
      <nav aria-label="Navegación principal" className="mt-8 flex flex-1 flex-col gap-1">
        {ITEMS_NAV.map(({ icono: Icono, etiqueta, activo }) => (
          <button
            key={etiqueta}
            type="button"
